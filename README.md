...
| **Two documents reporting the identical period are merged into a new combined document, never silently picked between or left as duplicates** | Keep whichever was extracted most recently as "latest" (the original, pre-PR-#54 behavior), or let a human manually pick one | Two independent extractions of the same real period (e.g. ADF Farm Solutions and the Information Document, both genuinely FY2025) is real complementary evidence, not noise — reusing whichever fields each source uniquely provided is more complete than discarding one. A genuine field-level conflict between the two is never resolved silently, though — it's tagged `needs_review` with both candidate values named, reusing the existing review/approve workflow. |
...
- **Automated tests**: 222 backend (pytest) + 186 frontend (Vitest) tests, run before every merge.
